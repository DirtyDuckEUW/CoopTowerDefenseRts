...
namespace UI.HUD
{
  public class ActionFrame : MonoBehaviour
  {
    ...
    {
      GameObject spawnedObject = Instantiate(spawnOrder[0], spawnPoint.transform.parent.position, Quaternion.identity);
      Units.Player.PlayerUnit pu = spawnedObject.GetComponent<Units.Player.PlayerUnit>();
      pu.transform.parent = Player.PlayerManager.instance.playerUnits.Find("Warriors");
      pu.baseStats = Units.UnitHandler.instance.GetBasicUnitStats("warrior");
